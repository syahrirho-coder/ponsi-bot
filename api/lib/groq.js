...
const SIGNAL_JSON_SPEC = `Balas HANYA dengan satu objek JSON valid, tanpa teks lain, tanpa markdown fence, dengan bentuk persis:
{
  ...
  "technical_summary": string (7-10 kalimat bahasa Indonesia, HARUS runtut membahas semua ini satu-satu pakai angka spesifik yang diberikan, bukan cuma disebut sekilas: (1) struktur tren dari EMA20/50/100 — apakah stacked bullish/bearish atau campur, (2) momentum dari RSI14 dan MACD (line/signal/histogram) — apakah konfirmasi atau divergen, (3) posisi harga terhadap Bollinger Bands dan Stochastic %K/%D — overbought/oversold atau netral, (4) ADX14 buat nilai apakah tren ini kuat atau choppy/sideways, (5) posisi funding rate & long/short ratio Binance/Bybit/OKX — searah atau berlawanan, dan apa artinya buat crowd positioning, (6) orderbook imbalance dan estimasi tekanan liquidation kalau tersedia, (7) berita/momentum burst kalau ada katalis. Tutup dengan 1 kalimat kesimpulan yang menyatukan semua poin di atas jadi alasan kenapa arah dan level entry/SL/TP ini masuk akal. Jangan sekadar menyebut nama indikator tanpa angka — selalu sertakan nilai persisnya supaya orang yang baca bisa mengecek sendiri ke chart),
  "invalidation": string (1-2 kalimat bahasa Indonesia, kondisi harga SPESIFIK dengan angka yang membatalkan sinyal ini, misal candle close di atas/bawah level tertentu atau indikator berbalik),
...
Aturan angka: entry, stop_loss, take_profit_1/2/3 harus konsisten dengan arah sinyal (untuk BUY: SL di bawah entry, TP1<TP2<TP3 di atas entry; untuk SELL kebalikannya), dan realistis terhadap harga & ATR yang diberikan. confidence harus konsisten dengan technical_summary — kalau banyak indikator saling konfirmasi searah, confidence boleh tinggi (70-90); kalau ada indikator yang saling bertentangan, confidence harus lebih rendah (40-60) dan technical_summary WAJIB menyebutkan pertentangan itu secara jujur, bukan ditutup-tutupi. Balas dalam format JSON.`;
...
async function callGroq({ system, userText, image, keyOffset = 0 }) {
  ...
  // Vercel's function timeout no matter how many keys we end up trying.
  // Vercel Hobby defaults to 10s (configurable up to 60s via maxDuration in
  // vercel.json — see analyze/analyze-dex there) — this MUST stay
  // comfortably under whichever duration applies, or Vercel kills the
  // function itself and the frontend gets an HTML/error page back instead
  // of the JSON error this abort path is meant to produce.
