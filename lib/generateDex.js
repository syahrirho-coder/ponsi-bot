...
const { fetchPoolOhlcvGT, STYLE_TO_GECKOTERMINAL } = require("./geckoterminal");
const { callGroq, SIGNAL_JSON_SPEC } = require("./groq");

// "swing" (H4) WAJIB lewat GeckoTerminal karena DexPaprika gak punya
// interval 4 jam asli sama sekali — jadi buat style ini GeckoTerminal
// adalah provider UTAMA, bukan fallback.
const GECKOTERMINAL_PRIMARY_STYLES = new Set(["swing"]);

// Minimum candle biar dianggap valid buat dianalisa (kurang dari ini =
// dianggap gagal, lanjut ke provider berikutnya).
const MIN_CANDLES = 30;

// Ambil candle dengan fallback: coba provider utama dulu, kalau gagal atau
// candle-nya kurang dari MIN_CANDLES (indikasi pool masih terlalu
// baru/kecil buat ke-index penuh), otomatis coba provider satunya sebelum
// benar-benar menyerah. Ini nutup celah DexPaprika yang indexing-nya kadang
// belum lengkap buat pool baru launch/likuiditas kecil — TAPI kalau pool-nya
// bener-bener baru terlalu baru, dua provider ini bisa aja SAMA-SAMA gagal,
// karena keduanya nunggu on-chain indexer yang beda infra masing-masing.
async function fetchCandlesWithFallback(chain, poolAddress, style) {
  const primaryIsGT = GECKOTERMINAL_PRIMARY_STYLES.has(style);
  const primary = primaryIsGT
    ? { name: "GeckoTerminal", fn: () => fetchPoolOhlcvGT(chain, poolAddress, style, 150), cfg: STYLE_TO_GECKOTERMINAL[style] }
    : { name: "DexPaprika", fn: () => fetchPoolOhlcv(chain, poolAddress, style, 150), cfg: STYLE_TO_GT[style] || STYLE_TO_GT.scalping };
  const fallback = primaryIsGT
    ? null // swing gak punya fallback lain — GeckoTerminal satu-satunya sumber H4 asli
    : { name: "GeckoTerminal", fn: () => fetchPoolOhlcvGT(chain, poolAddress, style, 150), cfg: STYLE_TO_GECKOTERMINAL[style] };

  let primaryError = null;
  try {
    const candles = await primary.fn();
    if (candles && candles.length >= MIN_CANDLES) {
      return { candles, source: primary.name, styleCfg: primary.cfg };
    }
    primaryError = new Error(`Candle ${primary.name} untuk pool ini cuma ${candles ? candles.length : 0} — kurang dari minimum ${MIN_CANDLES}.`);
  } catch (err) {
    primaryError = err;
  }
  ...
  try {
    const candles = await fallback.fn();
    if (candles && candles.length >= MIN_CANDLES) {
      return { candles, source: fallback.name, styleCfg: fallback.cfg };
    }
    throw new Error(`Candle ${fallback.name} untuk pool ini cuma ${candles ? candles.length : 0} — kurang dari minimum ${MIN_CANDLES}.`);
  } catch (fallbackError) {
    throw new Error(`${primary.name} gagal (${primaryError.message}) DAN ${fallback.name} juga gagal (${fallbackError.message}) — pool ini kemungkinan terlalu baru/kecil buat ke-index di kedua provider.`);
  }
}
...
async function generateDexSignal({ chain, chainLabel, poolAddress, symbol, style = "scalping", context = {}, keyOffset = 0 }) {
  ...
  const { candles, source: candleSource, styleCfg } = await fetchCandlesWithFallback(chain, poolAddress, style);
...
=== INDIKATOR TEKNIKAL (dari candle ${candleSource}) ===
...
Buat satu sinyal trading sesuai spesifikasi JSON.`;
  ...
  return {
    ...
    indicators: ind,
    signal,
    source: `${candleSource} (candle on-chain) + DexScreener (liquidity/mcap/volume)`,
