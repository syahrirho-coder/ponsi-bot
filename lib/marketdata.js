...
/* ---------------- Bybit Futures (candle utama) ---------------- */

// Normalizes user input ("btc", "BTC/USDT", "btcusdt") into a Bybit
// linear perpetual symbol ("BTCUSDT"). Defaults the quote to USDT when the
// user only typed a base asset, since that's the overwhelming majority of
// USDT-M perpetuals. Format sama persis kayak Binance (BTCUSDT), jadi
// fungsi ini tetap dipakai dobel — buat manggil Bybit (candle utama) MAUPUN
// Binance (salah satu sumber composite data di bawah).
function toFuturesSymbol(raw) {
...
// Style key (dipakai di seluruh app) -> interval kline Binance Futures.
...
// Candles diambil LANGSUNG dari Binance Futures — sempet dipindah ke
// CryptoCompare karena diasumsikan Binance ikut block kayak Bybit, tapi
// project Signalynx (akun Netlify yang sama) manggil endpoint ini langsung
// tanpa masalah, jadi dibalikin ke Binance biar datanya native/lengkap lagi
// (bukan cuma "n/a" seperti kalau Binance beneran diblock, ini SUMBER UTAMA
// candle buat semua indikator teknikal, harus reliable).
async function fetchFuturesKlines(symbol, style, limit = 150) {
...
/* ---------------- Binance Futures (composite data, opsional) ---------------- */

// Dipertahankan sebagai symbol normalizer lama biar kompatibel — sama
// persis isinya kayak toFuturesSymbol di atas (Binance & Bybit format sama).
function toBinanceFuturesSymbol(raw) {
  return toFuturesSymbol(raw);
}
...
/* ---------------- Solana wallet-level flow (Birdeye Top Traders) ----------------
 * Meteora only exposes pool-level aggregates (checked directly against its
 * OpenAPI spec — no per-wallet breakdown exists there). To answer "wallet
 * mana yang akumulasi/distribusi dan berapa jumlahnya", a different
 * provider is needed: Birdeye indexes individual trader activity per
 * token. This REQUIRES a Birdeye API key (BIRDEYE_API_KEY env var) — unlike
 * Meteora, Birdeye has no keyless tier. If the key isn't configured, this
 * silently returns null (feature disabled, everything else keeps working).
 * Endpoint confirmed against Birdeye's official reference docs
 * (docs.birdeye.so/reference/get-defi-v2-tokens-top_traders).
 */
const BIRDEYE_API = "https://public-api.birdeye.so";
const SOL_MINT = "So11111111111111111111111111111111111111112"; // wrapped SOL mint, used by Birdeye for native SOL

// Generalized version, usable for ANY Solana token mint address — not just
// native SOL. Used directly by lib/memecoin.js for gmgn.ai-style memecoins
// (pump.fun launches, small Solana tokens), and wrapped below by
// fetchSolanaWalletFlow(base) to keep the existing SOL-only call site
// working unchanged.
async function fetchWalletFlowForMint(mintAddress, opts = {}) {
  if (!mintAddress) return null;
  const apiKey = process.env.BIRDEYE_API_KEY;
  if (!apiKey) return null;

  const url = `${BIRDEYE_API}/defi/v2/tokens/top_traders?address=${mintAddress}&time_frame=24h&sort_type=desc&sort_by=volume&limit=10`;
  let res;
  try {
    res = await fetch(url, {
  ...
  const items = j?.data?.items || j?.data?.tokens || (Array.isArray(j?.data) ? j.data : []);
  if (!Array.isArray(items) || items.length === 0) return null;

  let totalBuyUsd = 0;
  let totalSellUsd = 0;
  const wallets = items.map((t) => {
    const buyUsd = Number(t.volumeBuyUSD ?? t.volumeBuyUsd ?? 0) || 0;
    const sellUsd = Number(t.volumeSellUSD ?? t.volumeSellUsd ?? 0) || 0;
    totalBuyUsd += buyUsd;
    totalSellUsd += sellUsd;
    const addr = t.address || t.owner || "";
    return {
      addressShort: addr ? `${addr.slice(0, 4)}...${addr.slice(-4)}` : "n/a",
      buyUsd,
      sellUsd,
      netUsd: buyUsd - sellUsd,
      holdVolumeUsd: Number(t.holdVolumeUsd ?? 0) || 0,
      tags: Array.isArray(t.tags) ? t.tags : [],
    };
  });

  return {
    source: "Birdeye Top Traders (Solana, 24h)",
    label: opts.label || "Top Trader SOL 24h (Birdeye)",
    walletCount: wallets.length,
    totalBuyUsd,
    totalSellUsd,
    netFlowUsd: totalBuyUsd - totalSellUsd,
    topWallets: wallets.sort((a, b) => Math.abs(b.netUsd) - Math.abs(a.netUsd)).slice(0, 5),
  };
}

/* ---------------- Solana token security (Birdeye Token - Security) ----------------
 * gmgn.ai-style risk panel: mint/freeze authority status, creator holding %,
 * top-10 holder concentration. Same BIRDEYE_API_KEY as fetchWalletFlowForMint
 * above — no new provider/key needed. Endpoint per Birdeye's official
 * reference (docs.birdeye.so/reference/get-defi-token_security).
 *
 * NOTE: field names below (creatorPercentage, top10HolderPercent, freezeable,
 * etc.) are Birdeye's documented Solana schema as of when this was written —
 * the interactive example response on their docs page is JS-rendered and
 * wasn't fetchable to double-check verbatim. Before relying on this in
 * production, fire one real request (Postman or curl with your key) and
 * diff the actual field names against what's read below; the safeJson
 * fallback chains (a ?? b ?? c) exist so small naming variants don't break.
 */
async function fetchTokenSecurity(mintAddress) {
  if (!mintAddress) return null;
  const apiKey = process.env.BIRDEYE_API_KEY;
  if (!apiKey) return null;

  const url = `${BIRDEYE_API}/defi/token_security?address=${mintAddress}`;
  let res;
  try {
    res = await fetch(url, {
      headers: { "X-API-KEY": apiKey, "x-chain": "solana", accept: "application/json" },
  ...
  const mintRevoked = d.mintAuthority === null || d.mintAuthority === undefined;
  const freezeRevoked = d.freezeAuthority === null || d.freezeAuthority === undefined;
  const top10Pct = d.top10HolderPercent ?? d.top10HolderRate ?? null;
  const creatorPct = d.creatorPercentage ?? d.creatorBalanceRate ?? null;
  const lpLocked = d.lpLockedPercentage ?? d.lockInfo?.lockedPercentage ?? null;

  return {
    source: "Birdeye Token Security (Solana)",
    mintAuthorityRevoked: mintRevoked,
    freezeAuthorityRevoked: freezeRevoked,
    top10HolderPercent: top10Pct != null ? Number(top10Pct) * (top10Pct <= 1 ? 100 : 1) : null,
    creatorPercent: creatorPct != null ? Number(creatorPct) * (creatorPct <= 1 ? 100 : 1) : null,
    lpLockedPercent: lpLocked != null ? Number(lpLocked) * (lpLocked <= 1 ? 100 : 1) : null,
    isToken2022: !!d.isToken2022,
    transferFeeEnabled: !!(d.transferFeeEnable ?? d.transferFeeData),
  };
}

// Thin wrapper kept for the existing "native SOL inside the crypto market"
// call site in fetchCompositeData below — unchanged behavior/shape.
async function fetchSolanaWalletFlow(base) {
  if (String(base || "").toUpperCase() !== "SOL") return null;
  return fetchWalletFlowForMint(SOL_MINT);
}
...
async function fetchCompositeData(rawSymbol) {
  ...
  const [
    ...
    fetchBtcDominance(),
    // Bybit composite endpoints dimatiin — infra Netlify kena block 403
    // di Bybit (lihat komentar di fetchFuturesKlines), jadi ini akan SELALU
    // null. Daripada nunggu 5 detik timeout percuma tiap kali klik Analisa,
    // langsung resolve null.
